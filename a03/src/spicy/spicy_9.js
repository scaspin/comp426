...
export const repeatDemo = () => {
    repeat(function print(text) {console.log(text)}, 10, "Hello, world!");
};
...
export const filter = (arr, test) => {
    ...
    arr.forEach(function(a){
        if (test(a)){
            r["pass"] = r["pass"].concat(a)
        }else{
            r["fail"] = r["fail"].concat(a)
        }
    });

    if (r["pass"].length == arr.length){
        return true;
    }
    if (r["fail"].length == arr.length){
        return false;
    }
...
// console.log(allEvensAreOdd([1,2,3,5,7,9,11]))
